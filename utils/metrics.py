...
def compute_traitors_game_metrics(csv_file):
        ...
        tas_r = df[df["Vote_Type"] == "Traitor"].groupby("Round")["Vote_Target"].apply(lambda x: x.value_counts().max() / len(x))
        tas = tas_r.mean()

        # Faithful Agreement Score (FAS)
        fas_r = df[(df["Vote_Type"] == "General") & (df["Role"] == "Faithful")].groupby("Round")["Vote_Target"].apply(lambda x: x.value_counts().max() / len(x))
        fas = fas_r.mean()

        # Faithful Correctness Rate (FCR)
        fcr_r = df[(df["Vote_Type"] == "General") & (df["Role"] == "Faithful")].groupby("Round")["Vote_Target"].apply(lambda x: (x.isin(traitors)).sum() / len(x))
        fcr = fcr_r.mean()
        ...
        tsr = (initial_traitors - eliminated_traitors) / initial_traitors if initial_traitors > 0 else 0
        ...
        fsr = (initial_faithfuls - eliminated_faithfuls) / initial_faithfuls if initial_faithfuls > 0 else 0

        # Deception Effectiveness Score (DES)
        def deception_success(group):
            eliminated_player = group["Eliminated"].iloc[0]
            return all(group["Vote_Target"] == eliminated_player) and eliminated_player in faithfuls

        des_r = df[df["Vote_Type"] == "Traitor"].groupby("Round").apply(deception_success)
        des = des_r.mean()

        # Information Diffusion Rate (IDR)
        idr_r = df[(df["Vote_Type"] == "General") & (df["Role"] == "Faithful")].groupby("Round")["Vote_Target"].apply(lambda x: (x.isin(traitors)).sum() / len(x))
        idr = idr_r.mean()
        ...
        def betrayal_recognition(group):
            ...
            return (traitor_votes["Vote_Target"] != max_voted).sum() / len(traitor_votes) if len(traitor_votes) > 0 else 0

        brr_r = df[(df["Vote_Type"] == "General") & (df["Role"] == "Faithful")].groupby("Round").apply(betrayal_recognition)
        brr = brr_r.mean()
