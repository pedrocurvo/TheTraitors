...
def validate_llm_config(llm_config: Dict[str, Any], config_name: str = "llm") -> None:
    ...
    for field, field_type in required_llm_fields.items():
        ...
        if not isinstance(llm_config[field], field_type):
            raise ValueError(
                f"Invalid type for {field} in {config_name}. Expected {field_type}"
            )
    ...
    if llm_config["client_type"] not in valid_client_types:
        raise ValueError(
            f"Invalid client_type in {config_name}. Must be one of: {valid_client_types}"
        )
...
def merge_config_with_args(
    config: Dict[str, Any], args: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
