...
from src.agent import Agent  # Import the Agent class
from src.llm_client import LLMClientFactory  # Import the LLM client factory
from src.traitors_game import (
    TraitorsGame,  # Import the TraitorsGame class from the new file
)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--traitors", type=int, help="Number of traitor agents (overrides config file)"
    )
    parser.add_argument(
        "--model", type=str, help="Model name to use (overrides config file)"
    )
    parser.add_argument(
        "--seed",
        type=int,
